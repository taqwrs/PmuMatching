...
function normalizeResults(results) {
  return Array.isArray(results)
    ? results
        .filter((item) => normalizeScore(item?.score) > 0)
...
function buildReportMeta({ proposalTitle, total }) {
  return {
    title: REPORT_TITLE,
...
<body>
  <main class="page">
    ...
    <p class="summary">จัดเรียงผลลัพธ์ตามคะแนนความเหมาะสมจากมากไปน้อย เพื่อใช้ประกอบการพิจารณาแหล่งทุนที่เกี่ยวข้องกับโครงการ</p>
    ${rowsHtml}
...
export function downloadMatchReportExcel({ results, proposalTitle }) {
  const rows = normalizeResults(results);
  const meta = buildReportMeta({ proposalTitle, total: rows.length });
  ...
  const fileName = `${slugifyFileName(proposalTitle || "match-report")}-top-3.xls`;
...
export function printMatchReportPdf({ results, proposalTitle }) {
  const rows = normalizeResults(results);
  const meta = buildReportMeta({ proposalTitle, total: rows.length });
