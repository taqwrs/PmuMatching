...
function cleanText(value, maxLength = 2000) {  // แก้จุดที่ 4: เปลี่ยน default เป็น 2000
...
เรียงผลลัพธ์จาก score สูงไปต่ำ`
    ...
    let raw
    try { raw = JSON.parse(modelContent) }
    catch { throw new Error('AI ส่งผลลัพธ์ที่ไม่ใช่ JSON') }

    if (!Array.isArray(raw.results)) throw new Error('รูปแบบผลลัพธ์จาก AI ไม่ถูกต้อง')
    ...
    // แก้จุดที่ 3: ส่งผลกลับก่อนเสมอ แล้วค่อยบันทึก DB แยกกัน
    const response = Response.json({ success: true, results })

    // แก้จุดที่ 2: บันทึกเฉพาะเมื่อมี title และมีอย่างน้อย 1 แหล่งทุนที่ได้คะแนน
    if (cleanProposalTitle) {
      const topScore = results[0]?.score ?? 0

      if (topScore >= MIN_SCORE_TO_SAVE) {
        try {
          const { data: proposal, error: proposalError } = await supabase
            .from('proposals')
            .insert({ title: cleanProposalTitle, content_text: cleanAbstract })
            .select()
            .single()
