...
9. ถ้ากรอบโจทย์/เงื่อนไขระบุคุณสมบัติผู้สมัครไว้ชัดเจน เช่น ต้องสังกัดหน่วยงานรัฐ ต้องมีตำแหน่งทางวิชาการ หรือต้องเป็นผู้ประกอบการ SME และไม่มีข้อมูลในชื่อโครงการ/abstract ที่บ่งชี้ว่าโครงการเข้าเกณฑ์นั้นได้ ให้ปรับคะแนนลงเหลือไม่เกิน 40 แม้แผนงานจะตรงกับเป้าหมายโครงการก็ตาม และต้องระบุเหตุผลนี้ใน reason_mismatch อย่างชัดเจน
10. score ต้องเป็นจำนวนเต็ม 0-100 และใช้คะแนนละเอียดทุกเลขได้ เช่น 81, 82, 83, 84, 86, 87, 88 ไม่จำกัดเฉพาะเลขลงท้าย 0 หรือ 5
...
function buildResults(fundings, allRawResults) {
  ...
  const allResults = fundings
    ...
    .sort((a, b) => b.score - a.score || a.funding_name.localeCompare(b.funding_name, "th"));
