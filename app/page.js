...
async function readResponse(response) {
  ...
  try {
    ...
    return { success: false, error: "เซิร์ฟเวอร์ตอบกลับข้อมูลไม่ถูกต้อง" };
  }
}
...
const styles = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header { padding: 2rem 1.25rem 1rem; max-width: 680px; margin: 0 auto; }
  ...
  .header h1 { margin-top: 12px; font-size: 22px; font-weight: 600; color: #1a1a18; line-height: 1.3; }
  .header p { margin-top: 6px; font-size: 14px; color: #5f5e5a; line-height: 1.6; }

  .nav-desktop { display: none; }
  @media (min-width: 768px) {
    .nav-desktop { display: flex; gap: 4px; max-width: 680px; margin: 0 auto 1.5rem; padding: 0 2rem; }
    .nav-desktop button { flex: 1; display: flex; align-items: center; justify-content: center; gap: 8px; padding: 10px 16px; border-radius: 10px; border: none; font-size: 14px; font-weight: 500; cursor: pointer; transition: all 0.15s; }
  ...
  .content { max-width: 680px; margin: 0 auto; padding: 0 1.25rem; }
  ...
  input[type="file"]::file-selector-button {
    padding: 7px 14px; background: #EEEDFE; color: #3C3489; border: none;
    border-radius: 7px; font-size: 13px; font-weight: 500; cursor: pointer; margin-right: 12px;
  }
  ...
  .file-status .status-name { display: flex; align-items: center; gap: 6px; font-size: 13px; font-weight: 500; color: #0F6E56; }
  .file-status .status-meta { font-size: 12px; color: #888780; margin-top: 6px; display: flex; gap: 12px; flex-wrap: wrap; }
  .reset-btn { display: flex; align-items: center; gap: 5px; padding: 5px 10px; border-radius: 6px; border: 0.5px solid #d3d1c7; background: #fff; font-size: 12px; color: #5f5e5a; cursor: pointer; transition: all 0.15s; white-space: nowrap; }
  ...
  .result-item { border: 0.5px solid #e0dfd8; border-radius: 12px; padding: 1rem 1.25rem; margin-bottom: 12px; background: #fff; transition: border-color 0.15s; }
  ...
  .reason-label { font-weight: 600; display: block; margin-bottom: 3px; font-size: 12px; text-transform: uppercase; letter-spacing: 0.04em; opacity: 0.7; }
        ...
        <div className="content">
          {tab === "match" && (
            <div className="card">
              ...
              <div className="field">
                ...
                <div className="mode-toggle">
                  ...
                  <button type="button" className={`mode-btn ${proposalMode === "pdf" ? "active" : ""}`}
                    onClick={() => { setProposalMode("pdf"); setProposalText(""); setMatchError(""); setMatchResults([]); }}>
                    <Icon.Upload /> แนบ PDF
                  </button>
                </div>
                    ...
                    {proposalFileName && !proposalLoading && (
                      <div className="file-status">
                        <div className="status-row">
                          <span className="status-name"><Icon.Check /> {proposalFileName}</span>
                          <button type="button" className="reset-btn" onClick={resetProposal}>
                            <Icon.Reset /> เริ่มใหม่
                          </button>
                        </div>
              ...
              {matchError && (
                <div className="error-box"><Icon.Alert /><span>{matchError}</span></div>
              )}
              ...
              {matchResults.length > 0 && (
                <>
                  ...
                  <div className="results-header">
                    <h3>ผลการจับคู่</h3>
                    <span className="count-badge">{matchResults.length} รายการ</span>
                  </div>
          ...
          {tab === "funding" && (
            <div className="card">
                ...
                {fundingMode === "text" ? (
                  ...
                  <div className="drop-zone">
                    ...
                    {fundingFile && (
                      <div className="file-status">
                        <div className="status-row">
                          <span className="status-name"><Icon.Check /> {fundingFile.name}</span>
                          <button type="button" className="reset-btn" onClick={resetFunding}>
                            <Icon.Reset /> เริ่มใหม่
                          </button>
                        </div>
                        <div className="status-meta">
                          <span>{(fundingFile.size / 1024).toFixed(0)} KB</span>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              {fundingError && (
                <div className="error-box"><Icon.Alert /><span>{fundingError}</span></div>
              )}
